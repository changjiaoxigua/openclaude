...
##备注，npm以外的ripgrep依赖：
1、 /vendor/ripgrep/x64-win32/rg.exe这个包是做什么的  
⏺ ripgrep (rg) 是 Rust 编写的代码搜索工具，用于：

- 文件搜索：快速查找项目中的文件（rg --files）
- 内容搜索：在代码中搜索字符串/正则（rg "pattern"）
- Glob 功能：支持 .gitignore 等过滤规则  
  YwCoder 中的用途：
- GrepTool：代码内容搜索
- GlobTool：文件列表获取
- AI 理解项目结构时用到  
  为什么打包：
- Windows 内网环境可能没有预装 rg
- 确保离线安装后功完整
  体积：约 5-6MB简单说，就是文件搜索的基础工具，没有它代码搜索功能会失效（当前打包版本仅打包windows-only，如需适配linux/mac，需要额外打包适配linux/mac的依赖  
  2、vendor/ripgrep 没有被打包进cli.mjs的原因：
  vendor/ripgrep/x64-win32/rg.exe 是一个 Windows PE 格式的可执行文件。和node modules里 sharp 的 native binary 一样，本质都是原生二进制文件无法嵌入 JS 文件。
  ripgrep没有单独放进nodemodules的原因：是因为rg.exe 是 Rust 写的独立可执行文件，没有对应的 npm 包；代码里是直接拼路径字符串来找到它：
  // 类似这样
  const rgPath = path.join(\_\_dirname, '../vendor/ripgrep/x64-win32/rg.exe')
  spawn(rgPath, args)
  它通过文件路径被调用，而不是通过 Node.js 模块解析
  是在 CI 里从 GitHub Releases 直接下载的，不走 npm install
  在 Claude Code 官方包的结构里，cli.js 和 vendor/ 是并列的兄弟目录——这是业界处理外部二进制工具的惯用做法。
